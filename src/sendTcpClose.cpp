...
void sendTcpClose(pcap_t* pcapH, u_char *packet, char *blockStr){
    ...
    // ▼ TCP_RST_PACKET (To. Server)
    tcpRstPacket.ep = *ethh;
    tcpRstPacket.iph = *iph;
    tcpRstPacket.iph.ip_len = htons((int)(sizeof(struct ip) + sizeof(struct tcphdr)));
    tcpRstPacket.iph.ip_sum = 0;
    tcpRstPacket.iph.ip_sum = htons(calcIpChksum(&(tcpRstPacket.iph)));
    tcpRstPacket.tcph = *tcph;
    tcpRstPacket.tcph.ack = 0;
    tcpRstPacket.tcph.syn = 0;
    tcpRstPacket.tcph.fin = 0;
    tcpRstPacket.tcph.psh = 0;
    tcpRstPacket.tcph.rst = 1;
    ipPseh.sIp = ntohl(tcpRstPacket.iph.ip_src.s_addr);
    ipPseh.dIp = ntohl(tcpRstPacket.iph.ip_dst.s_addr);
    ipPseh.protocol = tcpRstPacket.iph.ip_p;
    ipPseh.tcpLen = sizeof(struct tcphdr);
    tcpPseh.source_port = ntohs(tcpRstPacket.tcph.th_sport);
    tcpPseh.dest_port = ntohs(tcpRstPacket.tcph.th_dport);
    tcpPseh.sequence_number = ntohl(tcpRstPacket.tcph.th_seq);
    tcpPseh.ack_number = ntohl(tcpRstPacket.tcph.th_ack);
    tcpPseh.data_offset_reserved_flags = (tcpRstPacket.tcph.th_off << 12) + tcpRstPacket.tcph.th_flags;
    tcpPseh.window_size = ntohs(tcpRstPacket.tcph.th_win);
    tcpRstPacket.tcph.th_sum = calcTcpChksum(&ipPseh, &tcpPseh, NULL, 0);
    if (pcap_sendpacket(pcapH, (unsigned char*)&tcpRstPacket, sizeof(tcpPacket)) != 0){
        printf("%s\n", pcap_geterr(pcapH));
    }
